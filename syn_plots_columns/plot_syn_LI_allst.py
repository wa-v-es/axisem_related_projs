...
## for loyalty island..also change line 34/35
input_dir= '/Users/keyser/Research/axisem/loyalty_isl/input'
syn_dir = '/Users/keyser/Research/axisem/loyalty_isl/output_10sec_expl_source/stations/AK_81/sac_bf/'
syn_dir = '/Users/keyser/Research/axisem/loyalty_isl/output_10sec_dirac/stations/AK_81/sac_bf/'
...
for j in range(3):
    ...
    ax[j].xaxis.set_major_locator(MultipleLocator(200))
    ...
    for i in range(len(st_temp)):
        ...
        arr_PP=arrivals[0]
        # start = eq origin+PP-400
        # sys.exit()
        # auto.trim(starttime=origin_time+arr_P.time-20,endtime=origin_time+arr_P.time+60)
        auto.trim(starttime=origin_time+750,endtime=origin_time+1400)

        ax[j].scatter(arr_P.time-750, i+1,s=3,marker='o',facecolor='dimgray', edgecolor='black',alpha=.95,linewidth=.15,zorder=10)
        # ax[j].scatter(20, i+1,s=4,marker='o',facecolor='maroon', edgecolor='black',alpha=.95,linewidth=.15,zorder=10)
        ...
        l1,=ax[j].plot(time,auto.data + i+1,  lw=0.3, color='crimson',label='Syn')
        ####
...
plt.title('10 sec seis for LI event dirac - iasp91',loc='center')

# fig.text(0.52, 0.03, 'Seismograms centered at P-wave arrival (at 20sec; red dots)',fontsize=10, ha='center', va='center')
fig.text(0.52, 0.03, 'Time after event 750s + ',fontsize=10, ha='center', va='center')

plt.show()
#
# plt.savefig('LI_syn/10sec_syn_dirac_long.png',dpi=300,bbox_inches='tight', pad_inches=0.1)
sys.exit()
############
