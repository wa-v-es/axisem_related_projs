...
for j in range(3):
    ...
    for i in range(len(st_temp)):
        ...
        auto.trim(starttime=origin_time,endtime=origin_time+1000)

        ax[j].scatter(arr_P.time, i+1,s=4,marker='o',facecolor='maroon', edgecolor='black',alpha=.95,linewidth=.15,zorder=10)
        ...
        l1,=ax[j].plot(time,auto.data + i+1,  lw=0.4, color='teal',label='Syn')
        ####
...
fig.text(0.52, 0.03, 'Time after event',fontsize=10, ha='center', va='center')
