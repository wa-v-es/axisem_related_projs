...
syn_dir = '/Users/keyser/Research/axisem/loyalty_isl/output_10sec_2HD/stations/AK_81/sac_bf/'
...
for ist, st in enumerate(info_5deg):
    ...
    st_syn.filter('lowpass',freq=.1)
    st_iris.filter('lowpass',freq=.1)
...
for j in range(3):
    ...
    for i in range(len(st_temp)):
        ...
        auto.trim(starttime=origin_time+700,endtime=origin_time+1250)
        ...
        auto_s.trim(starttime=origin_time+700,endtime=origin_time+1250)
...
fig.text(0.52, 0.03, 'Time after event (s+700) lowpass (0.1 Hz)',fontsize=11, ha='center', va='center')
# plt.show()
#
plt.savefig('AK81_real_syn_HD2s_10mesh.png',dpi=300,bbox_inches='tight', pad_inches=0.1)
sys.exit()
############
